...
import { Colors, decryptData, encryptAmount } from '@/constants/Colors'
...
export default function addmoneycard({
    ...
    const SuccessHandler = async (res: any) => {
        ...
        try {
            setisloading(true);
            const response = await walletupdate(user?.customer_id, decryptData(token), encryptAmount(amount));
