...
import { getVFDVirtualAccountCustomerMaterial, validatetransaction, vfdvalidatetransaction } from '@/hooks/AuthRoutes'
...
export default function virtualaccounttopup({
    ...
    useEffect(() => {
        const unsubscribe = navigation.addListener('focus', async () => {
            ...
            try {
                const response = await getVFDVirtualAccountCustomerMaterial(
                    user?.customer_id,
                    encryptData(amount),
                    requestid,
                    decryptData(token)
    ...
    return (
        <SafeAreaView style={{ flex: 1, paddingHorizontal: 20, paddingTop: 10, backgroundColor: color1 }} edges={['top', 'bottom']}>
            <Animated.ScrollView showsVerticalScrollIndicator={false}>
                ...
                <FullScreenModal
                    visible={visible1}
                    onClose={() => [setIsVisible1(false), router.push({ pathname: '/(protected)/viewmaterials', params: { requestid: requestid } })]}
