...
import { validateTv } from '@/components/validateTv'
...
export default function billspaymentTv({
  ...
  const handleValidation = () => {
    ...
    // Correct condition
    if (formData.platform === "DSTVR" || formData.platform === "GOTVR") {
      validationErrors = validateTvRenewal(formData);
    } else {
      validationErrors = validateTv(formData);
    }
  ...
  const validatehandler = async () => {
    try {
      ...
      setFormData(prev => ({
        ...
        rprice: response.data.dueAmount,
  ...
  const makepayment = async () => {
    try {
      ...
      if (["DSTVR", "GOTVR"].includes(formData.platform)) {
        // Call endpoint A
        response = await tvrenewalpay(
          formData.reference,
          encryptData(formData.rprice),
          decryptData(token),
          formData.commission
        );
      } else {
        // Call endpoint B
        response = await tvpay(
          formData.reference,
